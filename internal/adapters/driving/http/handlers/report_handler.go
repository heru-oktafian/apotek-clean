...
func (h *ReportHandler) GetNeracaSaldo(c *fiber.Ctx) error {
	db := configs.DB
	...
	for _, s := range summaries {
		entry := fiber.Map{"transaction_type": s.TransactionType, "transaction_date": s.TransactionDate, "total_transaction": s.Total}
	...
	return helpers.JSONResponse(c, fiber.StatusOK, "Success", fiber.Map{"debit": debit, "credit": credit, "total_debit": totalDebit, "total_credit": totalCredit, "total_saldo": totalSaldo})
}

func (h *ReportHandler) GetProfitGraphByMonth(c *fiber.Ctx) error {
	db := configs.DB
	branchID, _ := services.GetBranchID(c)
	month := c.Query("month")
